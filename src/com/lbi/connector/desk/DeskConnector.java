...
public class DeskConnector extends Connector implements GeneralConstants, DeskConnectorConstants, ConnectorConstants, ConnectorXMLConstants
{

	public DeskConnector() throws Exception
	{
		super(DESK_CONN_ID);
	}
	
	public DeskConnector(Long connectorId) throws Exception
	{
		super(DESK_CONN_ID);
	}
	...
	private void handleResponse(ModuleInfo modInfo, int mode, Request req, CloseableHttpResponse response) throws Exception
	{
		if(respCode == 429)
		{
			if(response.containsHeader("X-Rate-Limit-Reset"))
			{
				Header[] retryAfter = response.getHeaders("X-Rate-Limit-Reset");
